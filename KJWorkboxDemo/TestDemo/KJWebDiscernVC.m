...
//遵循WKScriptMessageHandler协议，必须实现如下方法，然后把方法向外传递 通过接收JS传出消息的name进行捕捉的回调方法
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message{
    ...
    //JS调用OC
    if([message.name isEqualToString:@"changeColor"]){
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    NSLog(@"发送跳转请求：%@",urlStr);
//    //自己定义的协议头
//    NSString *htmlHeadString = @"https://www.jianshu.com";
...
- (void)dealloc{
    //移除注册的js方法
    [[_webView configuration].userContentController removeScriptMessageHandlerForName:@"changeColor"];
}
...
- (WKWebView *)webView{
    if(_webView == nil){
        ...
        [wkUController addScriptMessageHandler:weakScriptMessageDelegate name:@"changeColor"];
...
//        //设置缓存的请求策略和超时时间
//        [_webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:self.url] cachePolicy:(NSURLRequestReloadIgnoringLocalCacheData) timeoutInterval:(10.0f)]];
        
        NSString *path = [[NSBundle mainBundle] pathForResource:@"htmltest.html" ofType:nil];
